...
|---|---|---|---|
| KQ | Quality / 质量 | 0.15 | confidence × 0.7 + content_length_bonus × 0.3 |
| KG | Growth / 生长 | 0.12 | exponential decay from `updated_at` (24h half-life) |
| KA | Assetization / 资产化 | 0.13 | tags count × 0.15 + source_file bonus 0.3 |
| KM | Metabolism / 代谢 | 0.12 | `last_checked_at` recency (7d=1.0, 30d=0.0) |
| KD | Density / 密度 | 0.13 | inverse of link count (0=1.0/orphan, 5+=0.0/healthy) |
| KC | Retrieval / 检索 | 0.10 | embedding presence 0.8 + content length bonus 0.2 |
| KR | Response / 回答 | 0.15 | content structure (code, refs, headings) + length |
| KB | Block / 阻断 | 0.10 | pending error count (0=0.0, 1=0.5, 3+=1.0) |
