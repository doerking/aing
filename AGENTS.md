...
**正确口径**：170 是**主包**（发起部署的那个包）的面；各院本地面只许 **≥ 基线**——小于＝有档不在本院（真漂移），大于＝院属档（合法，但文档必须声明「院属档另计」，且门禁把差值一并印进读数）。
