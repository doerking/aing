 ...
 *
 * 实际配置文件：growth.config.js（位于 src/ 目录）
 * 本文件仅作为参考，展示配置结构。
 *
 * 注意：实际代码使用 kespi / jiezi / triPath 三个配置段。
 * 没有 paths / ai / pruning / performance 段。
 * triPath 段由 src/growth.config.js 末尾追加（envNum 辅助函数支持 TRI_PATH_TH_* 环境变量覆盖；
 * kespiPass 同源绑定 kespi.yellowLight）。本示例展示的是 kespi 段结构。
 */
