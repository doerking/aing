...
function markEntityKespiComputed(entityId, result) {
  const entityPath = path.join(__dirname, '..', 'wiki', 'entities', entityId + '.md');
  if (!fs.existsSync(entityPath)) return;
  let content = fs.readFileSync(entityPath, 'utf8');
  content = content.replace(/^kespi_status:.*$/m, 'kespi_status: computed');
  content = content.replace(
    /## KESPI 状态[\s\S]*$/,
    '## KESPI 状态\n\n```json\n' + JSON.stringify({ status: 'computed', overall: result.overall, dimensions: result.dimensions }, null, 2) + '\n```\n'
  );
...
class KespiChecker {
  ...
  async evaluateAll() {
    ...
    for (const entity of entities) {
      ...
      // P1b: 确保元数据行存在再更新；首算完成即视为 computed
      this.store.run(
        `INSERT OR IGNORE INTO entity_metadata (entity_id, kespi_score, last_checked_at) VALUES (?, ?, datetime('now'))`,
      ...
      try {
        markEntityKespiComputed(result.entity_id, result);
      } catch (e) { /* 文件同步失败不影响评分落库 */ }
    }
