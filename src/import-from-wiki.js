...
async function importEntities(baseDir) {
  ...
  if (!fs.existsSync(entitiesDir)) {
    console.log('❌ wiki/entities/ 目录不存在 / wiki/entities/ directory not found');
    store.close();
    return;
  }
  ...
  console.log(`📦 开始导入 ${files.length} 个实体... / Importing\n`);
  ...
  console.log(`\n✅ 导入完成: ${imported} 成功, ${skipped} 失败 / Import done:`);
  
  const stats = store.getStats();
  console.log(`\n📊 知识库统计: / Knowledge base stats:`);
  console.log(`   实体总数: ${stats.entities} / Total entities:`);
  console.log(`   链接总数: ${stats.links} / Total links:`);
